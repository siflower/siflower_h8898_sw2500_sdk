...
{
	struct ovl_inode *oi = kmem_cache_alloc(ovl_inode_cachep, GFP_NOFS);
...
static int ovl_parse_opt(char *opt, struct ovl_config *config)
{
	...
	config->redirect_mode = kstrdup(ovl_redirect_mode_def(), GFP_NOFS);
...
{
	int err = -ENOMEM;
	char *tmp = kstrdup(name, GFP_NOFS);
			  ...
			  struct ovl_layer *layers)
{
	...
	err = -ENOMEM;
	ofs->fs = kcalloc(numlower + 1, sizeof(struct ovl_sb), GFP_NOFS);
	if (ofs->fs == NULL)
		goto out;
				...
				struct ovl_fs *ofs, struct ovl_layer *layers)
{
	...
	stack = kcalloc(numlower, sizeof(struct path), GFP_NOFS);
	if (!stack)
...
static int ovl_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	err = -ENOMEM;
	ofs = kzalloc(sizeof(struct ovl_fs), GFP_NOFS);
	if (!ofs)
		goto out;
	...
	err = -ENOMEM;
	splitlower = kstrdup(ofs->config.lowerdir, GFP_NOFS);
	if (!splitlower)
		goto out_err;
	...
	layers = kcalloc(numlower + 1, sizeof(struct ovl_layer), GFP_NOFS);
	if (!layers)
		goto out_err;
