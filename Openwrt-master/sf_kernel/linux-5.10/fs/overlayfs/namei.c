				 ...
				 enum ovl_xattr ox)
{
	...
	fh = kzalloc(res + OVL_FH_WIRE_OFFSET, GFP_NOFS);
	if (!fh)
			...
			struct dentry *upperdentry, struct ovl_path **stackp)
{
	...
	if (!*stackp)
		*stackp = kmalloc(sizeof(struct ovl_path), GFP_NOFS);
	if (!*stackp) {
		dput(origin);
		return -ENOMEM;
	}
...
int ovl_verify_index(struct ovl_fs *ofs, struct dentry *index)
{
	...
	fh = kzalloc(len + OVL_FH_WIRE_OFFSET, GFP_NOFS);
	if (!fh)
		goto fail;
...
static int ovl_get_index_name_fh(struct ovl_fh *fh, struct qstr *name)
{
	...
	n = kcalloc(fh->fb.len, 2, GFP_NOFS);
	if (!n)
		return -ENOMEM;
			  ...
			  unsigned int flags)
{
	...
	if (upperdir) {
		...
		if (d.redirect) {
			err = -ENOMEM;
			upperredirect = kstrdup(d.redirect, GFP_NOFS);
			if (!upperredirect)
				goto out_put_upper;
			if (d.redirect[0] == '/')
				poe = roe;
