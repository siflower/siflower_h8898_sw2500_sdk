...
DEFINE_PER_CPU(ssize_t, total_page_frag_alloc_mem);
					     ...
					     gfp_t gfp_mask)
{
	...
	if (nc->va)
		this_cpu_add(total_page_frag_alloc_mem, 1 << compound_order(page));
...
void __page_frag_cache_drain(struct page *page, unsigned int count)
{
	...
	if (page_ref_sub_and_test(page, count)) {
		this_cpu_sub(total_page_frag_alloc_mem, 1 << compound_order(page));
