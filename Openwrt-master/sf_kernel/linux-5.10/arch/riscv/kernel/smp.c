...
struct sse_event *sse_backtrace;
int sse_backtrace_handler(u32 event_num, void *arg, struct pt_regs *regs)
{
	static DEFINE_RAW_SPINLOCK(nmi_backtrace_lock);

	raw_spin_lock(&nmi_backtrace_lock);
	nmi_cpu_backtrace(regs);
	raw_spin_unlock(&nmi_backtrace_lock);
	return 0;
}
#endif
...
static void riscv_backtrace_ipi(cpumask_t *mask)
{
	...
	if (sse_backtrace) {
		...
		for_each_cpu(cpu, mask)
			sbi_ecall(SBI_EXT_SSE, SBI_SSE_EVENT_SIGNAL,
				  SBI_SSE_EVENT_LOCAL_SOFTWARE,
				  cpuid_to_hartid_map(cpu), 0, 0, 0, 0);
		return;
	}
#endif
