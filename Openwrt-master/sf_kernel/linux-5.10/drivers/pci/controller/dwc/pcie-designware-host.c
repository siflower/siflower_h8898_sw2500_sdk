...
static void dw_handle_split_msi_irq(struct pcie_port *pp, const u8 idx)
{
	int pos, irq;
	unsigned long status;
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);

	status = dw_pcie_readl_dbi(pci, PCIE_MSI_INTR0_STATUS + (idx * MSI_REG_CTRL_BLOCK_SIZE));

	for_each_set_bit(pos, &status, MAX_MSI_IRQS_PER_CTRL) {
		irq = irq_find_mapping(pp->irq_domain, (idx * MAX_MSI_IRQS_PER_CTRL) + pos);
		generic_handle_irq(irq);
	}
...
static void dw_chained_split_msi_isr(struct irq_desc *desc)
{
	struct irq_chip *chip = irq_desc_get_chip(desc);
	struct pcie_port *pp;
	u8 *idx;
	...
	idx = irq_desc_get_handler_data(desc);
	pp = container_of(idx, struct pcie_port, msi_irq_idx[*idx]);
	dw_handle_split_msi_irq(pp, *idx);
		...
		if (!pp->ops->msi_host_init) {
			...
			if (ret == 0) {
				for (ctrl = 0; ctrl < num_ctrls; ctrl++) {
					pp->msi_irq_idx[ctrl] = ctrl;
					irq_set_chained_handler_and_data(pp->msi_irq[ctrl],
									 dw_chained_split_msi_isr,
									 &pp->msi_irq_idx[ctrl]);
				}
			} else {
