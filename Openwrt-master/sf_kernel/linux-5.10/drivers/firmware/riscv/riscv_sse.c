...
static int __init sse_backtrace_init(void)
{
	int ret = 0;
#ifdef CONFIG_SMP
	extern sse_event_handler sse_backtrace_handler;
	extern struct sse_event *sse_backtrace;
	struct sse_event *evt;

	evt = sse_event_register(SBI_SSE_EVENT_LOCAL_SOFTWARE, 0, sse_backtrace_handler, NULL);
	if (IS_ERR(evt))
		return PTR_ERR(evt);
