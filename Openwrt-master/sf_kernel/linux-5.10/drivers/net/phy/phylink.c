...
static void phylink_pcs_disable(struct phylink *pl)
{
	if (pl->pcs_ops && pl->pcs_ops->pcs_disable)
		pl->pcs_ops->pcs_disable(pl->pcs);
}

static int phylink_pcs_enable(struct phylink *pl)
{
	int err = 0;

	if (pl->pcs_ops && pl->pcs_ops->pcs_enable)
		err = pl->pcs_ops->pcs_enable(pl->pcs);
				  ...
				  const struct phylink_link_state *state)
{
	...
	if (pl->pcs_ops) {
		if (pl->pcs_state == PCS_STATE_STARTING)
			err = phylink_pcs_enable(pl);
