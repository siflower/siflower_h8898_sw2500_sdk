	...
	var ui_class = {
		...
		get_vendor: function(){
			...
			L.uci.callLoad('luci2').then(function(data){
				if(data && data.main){
					return L.globals.language = data.main.lang
				}else{
					return L.globals.language = "en"
				}
			})

			L.ui.callOption('basic_setting', 'vendor', 'name').then(function (value) {
				if (value.length <= 1){
					L.globals.vendorName = "default";
				}else{
					L.globals.vendorName = value.toLowerCase();
				}

				$.ajax(L.globals.resource + `/vendor/${L.globals.vendorName}/config.json`,{
					cache:    true,
					dataType: 'text',
					success:  function(data) {
						L.globals.vendor = JSON.parse(data);

						const { primaryColor, secondaryColor } = L.globals.vendor.color;

						if (primaryColor) {
							document.documentElement.style.setProperty('--primary-color', primaryColor);
						}

						if (Array.isArray(secondaryColor) && secondaryColor.length === 2) {
							document.documentElement.style.setProperty(
								'--secondary-color',
								`linear-gradient(to right, ${secondaryColor[0]}, ${secondaryColor[1]})`
							);
						} else {
				...
				document.documentElement.style.setProperty('--background-image', `url(../vendor/${L.globals.vendorName}/images/spriteImage.png)`);
			})
			return deferred;
		},

		set_vendor:function(name, elem) {
			const attribute = L.globals.vendor[`${name}_${L.globals.language}`];
			if (attribute) {
				$(elem).text(attribute);
			}
		},
		...
		login: function(invalid){
			...
			L.uci.callLoad('easymesh').then(function(res){
				if(Object.keys(res).length === 0){
					L.globals.easymesh = false
				}else{
					L.globals.easymesh = true
				}
			})
					...
					if (access)
					{
						...
						L.uci.load(['wireless', 'network']).then(function () {
							...
							if(L.uci.get('network', 'wan', 'proto') == 'bridge'){
								L.globals.bridge = true;
							}else{
								L.globals.bridge = false;
							}
							L.ui.loadTheme();
						});
					}
			...
			$.ajax(L.globals.resource + `/template/login_${L.globals.vendorName}.htm`, {
				cache:    true,
				dataType: 'text',
				success:  function(data) {
					data = data.replace(/<%([#:=])?(.+?)%>/g, function(match, p1, p2) {
					...
					L.ui.set_vendor("corporateName",".logo>div");
					L.ui.set_vendor("maintitle",".main_title");
					L.ui.set_vendor("footerInfo",".footer>div");

					L.ui.showLangSelect();

					$('#userpassword_ctrl').keypress(function (ev) {
		...
		callMenuCallback: function(entries){
			if (L.globals.wds || L.globals.bridge || !L.globals.easymesh) {
				for (const now in entries) {
					const check = now.includes('/') ? now.split('/')[0] : now;

					// startsWith  Compatibility with custom names
					if (L.globals.wds) {
						if (
							check === 'advance' ||
							check === 'network' ||
							now.startsWith('wireless/adv_wifi') ||
							now.startsWith('wireless/guest_wifi') ||
							now.startsWith('wireless/visit_control')
						) {
							delete entries[now];
						}
					}

					if (L.globals.bridge && now.startsWith('wireless/guest_wifi')) {
						delete entries[now];
					}

					if(!L.globals.easymesh){
						if (
							check === 'easymesh' ||
							now.startsWith('easymesh/basic') ||
							now.startsWith('easymesh/topology')
						) {
							delete entries[now];
						}
					}
...
//					L.ui.loading(false);
		...
		callGet: L.rpc.declare({
			object: 'uci',
			method: 'get',
			params: ['config', 'section', 'option'],
		}),

		updateWirelessBlackList: function (_mac, add) {
			var self = this;
			let mac = _mac.split("_").join(":");
			var mac0 = "wireless.default_radio0.maclist=" + mac;
			var mac1 = "wireless.default_radio1.maclist=" + mac;
			if(add){
				L.file.exec('uci', ['set', "wireless.default_radio0.macfilter=deny"]);
				L.file.exec('uci', ['set', "wireless.default_radio1.macfilter=deny"]);
				L.file.exec('uci', ['add_list', mac0]);
				L.file.exec('uci', ['add_list', mac1]);

			}else{
				L.file.exec('uci', ['del_list', mac0]);
				L.file.exec('uci', ['del_list', mac1]);
			}
			L.file.exec('uci', ['commit'])
		},
			...
			p.save = function(sid){
				...
				if(value == 'pppoe'){
					...
					L.uci.load('wireless').then(function() {
						L.uci.set("wireless", "guest_radio0", "disabled", "1");
						L.uci.set("wireless", "guest_radio1", "disabled", "1");
					});
		...
		loadBasewifi: function(){
			...
			e = s.option(L.cbi.DummyValue, '__encryption_warning', {
				caption:        L.tr(""),
			...
			e2 = s2.option(L.cbi.DummyValue, '__encryption_warning', {
				caption:        L.tr(""),
				initial:        L.tr("Wi-Fi is not encrypted, and there is a risk of theft by others, so please set a Wi-Fi password in time.")
			}).depends('encryption', function(v) {
				return (v == 'none');
			...
			self.renderMainMenu().then(function () {
				...
				$.ajax(L.globals.resource + '/template/theme.htm', {
					...
					success: function (data) {
						...
						L.ui.set_vendor("corporateName",".logo>div");
						L.ui.set_vendor("footerInfo","#footername");

						L.ui.callGet('network', 'wan', 'proto').then(function(proto) {
					...
					const vendorVersionMap = {
						"juovi": function() {
							L.ui.callOption('basic_setting', 'ota', 'version').then(function (version) {
								$('#theme_version').text(L.tr("Software Version:") + " V" + (version));
							});
						},
						"hsg": function(){
							vendorVersionMap.default()
						},
						"default": function() {
					...
					const handler = vendorVersionMap[L.globals.vendorName] || vendorVersionMap.default;
					handler();
					...
					$('#more').click(function () {
						self.changeView();
						L.setHash('view', L.globals.menuName);
					});
		...
		loadDevice: function() {
			...
			var set_spl =  L.rpc.declare({
				object: 'dpns.l2',
				method: 'spl',
				params: ['mac', 'vlan_id', 'spl_index', 'spl_mode', 'spl_scredit', 'spl_dcredit'],
			})

			function update_device_speed(device_data){
				...
				get_device_speed().then(function(speeds){
					for(let dev_mac in speeds){
						var mac = speeds[dev_mac].macaddr.replace(/:/g, '_').toUpperCase();
						var mac_id = speeds[dev_mac].macaddr = speeds[dev_mac].macaddr.replace(/:/g, '_').toUpperCase()+'_downspeed';
						var down_speed = Number(Math.round(speeds[dev_mac].downs / 1024 / 8))
						var speedlimit = device_data[mac].limitdown ? device_data[mac].limitdown : '0';
						if(speedlimit == "-1"){
							speedlimit = "0";
						}
						if(speedlimit != "0" && down_speed > speedlimit) {
							down_speed = speedlimit;
						}
			...
			function list_device(dev_name, data, processedMacs) {
				L.uci.callLoad(dev_name).then(function(results){
					for(let dev_mac in results){
						if((results[dev_mac].online == "1")||(results[dev_mac].internet == "0")){
							...
							sw_btn.click(function(){
								...
								if(forbidden == "1"){
									...
									do_cmd(cmd);
									self.updateWirelessBlackList(dev_mac,false);
									self.command("wifi reload").then(function () {
										L.ui.setting(false);
									});
								} else {
									L.uci.callSet(dev_name, dev_mac, {"internet":"0"});
									L.uci.callSet(dev_name, dev_mac, {"online":"1"});
									L.uci.callCommit(dev_name);
									...
									do_cmd(cmd);
									self.updateWirelessBlackList(dev_mac,true);
									self.command("wifi reload").then(function () {
										L.ui.setting(false);
									});
								}
							...
							$('#' + dev_mac + '_changerateId').click(function () {
								...
								$('#' + dev_mac + '_inputlimit').blur(function () {
									...
									var speedlimit = get_speedlimit(results, dev_mac);
									var vlan_id = results[dev_mac]["vlan_id"];
									var spl_index = parseInt(results[dev_mac]["mib_index"], 10);
									var spl_mode;
									...
									var spl_dcredit = Math.floor(new_speedlimit * 1000 / 64);
									console.log(spl_index)
									console.log(spl_dcredit)
									set_spl(mac, vlan_id, spl_index, spl_mode, 0, spl_dcredit).then(function(){
										L.ui.setting(false);
									});
									L.ui.loadDevice();
								});
			...
			$.ajax(L.globals.resource + '/template/device.htm', {
				...
				get_device_data().then(function(){
					...
					for(let m in device_data){
						if((device_data[m].online == "1")||(device_data[m].internet == "0")){
							n = n + 1;
						}
					}
					device_num = n;
				...
				self.deice_interval = setInterval(function () {
					//update ip and speed
						update_device_ip();
						update_device_speed(device_data);
