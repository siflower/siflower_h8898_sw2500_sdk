L.ui.view.extend({
	...
	execute: function () {
		...
		function connect(info) {
			...
			var btn_group = $('<div class="operaBtn" />')
				.css({
					'margin-top': '14px',
					position: 'relative',
					width: '735px',
			...
			self.wifi_status(L.globals.ifname).then(function (status) {
				...
				if(status['channel']){
					if(en_count == 0) {
						...
						clearInterval(ifwi1);
						$('body').find('.loading').remove();
						$('body').append(loadbox(L.tr("Connect success, ready to start Wi-Fi Repeater. Please refresh page two minutes later. If you cannot return the now page means the Wi-Fi Repeater connect success.After successful relay, please enter the IP address after relay to log in.")));
