L.ui.view.extend({
	...
	tmp_bin_info: L.rpc.declare({
		object: 'web.advance',
		method: 'get_tmp_bin_info',
		params: ['info']
	}),
	...
	need_upgrade: function(ota_version) {
		var self = this;
		return new Promise(function(resolve, reject) {
			self.get_img_version().then(function(data) {
				const versionRegex = /V(\d+)\.(\d+)\.(\d+)/;

				const img_ver = data.match(versionRegex);
				const ota_ver = ota_version.match(versionRegex);

				if (img_ver && ota_ver) {
					if (parseInt(ota_ver[1]) > parseInt(img_ver[1])) {
						resolve(true);
					} else if (parseInt(ota_ver[1]) < parseInt(img_ver[1])) {
						resolve(false);
					} else {
						if (parseInt(ota_ver[2]) > parseInt(img_ver[2])) {
							resolve(true);
						} else if (parseInt(ota_ver[2]) < parseInt(img_ver[2])) {
							resolve(false);
						} else {
							if (parseInt(ota_ver[3]) > parseInt(img_ver[3])) {
								resolve(true);
							} else {
								resolve(false);
							}
	...
	ota_upgrade: function () {
		var self = this;
		self.callShow('basic_setting').then(function (value) {
			var data = { };

			data.device_model = value.values.ota.romtype
			var cloud_code_url = value.values.ota.url + "/ota/files/getLatestByDevice"
			var data_query = $.param(data);
			$.ajax({
				method: "GET",
				url: cloud_code_url + "?" + data_query,
				success: function (data_get) {
					if(data_get.data == undefined || data_get.data.downloadUrl == undefined || data_get.data.md5Str == undefined || data_get.data.name == undefined || data_get.data.version == undefined) {
						...
						var downloadUrl = data_get.data.downloadUrl;
						var md5Str = data_get.data.md5Str;
						var filename = data_get.data.name;
						var version = data_get.data.version;
						L.ui.download()
						self.cleanUpgrade().then(function() {
							self.need_upgrade(version).then(function(need_upgrade) {
								if(!need_upgrade) {
									...
									.then(function(data) {
										if (data != "success") {
											...
											self.ota_upgrade_start(md5Str, filename, version);
										}
									})
								}
							})
				...
				error: function () {
					self.log_write('Failed to connect to server, please retry.');
					L.ui.dialog(
						L.tr(''), [
						$('<p />').text(L.tr('ota version get failed.')),
					],
	...
	ota_upgrade_start: function(ota_checksum, ota_img_name, ota_version) {
		var self = this;
		self.tmp_bin_info().then(function(info){
				var size = Number(info['info'].size);
				var md5sum = info['info'].md5sum
				if(md5sum != undefined && ota_checksum == md5sum)
				{
					var form = [
						...
						$('<ul />')
							.append($('<li />')
							.append($('<strong />').text(L.tr('IMG_name') + ': '))
							.append(ota_img_name))
							.append($('<li />')
							.append($('<strong />').text(L.tr('Checksum') + ': '))
							.append(ota_checksum))
