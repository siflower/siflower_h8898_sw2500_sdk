L.ui.view.extend({
	...
	execute: function() {
		...
		self.testReset().then(function(reset_avail) {
			...
			self.callLoad('basic_setting', 'ota', 'version').then(function (version) {
				$('#sw_ver').html("&nbsp;V" + version);
			});

			self.getFactoryInfo('hw_ver').then(function (hw_ver) {
				$('#hw_ver').html("&nbsp;V" + hw_ver["result"]);
			});
