	...
	getInfo: function(cb) {
		...
		return L.rpc.flush().then(function(info) {
			const rv = {
				...
				...info[1],
				...info[2],
				software_ver: info[1].release.description.split('openwrt_')[1],
				hw_ver: info[1].model,
				macaddr: info[3].result,
				model_ver: info[4].result,
				sn: info[6].result,
				login_domain: info[7],
				lan_ip: info[8]["ipv4-address"]?.[0]?.address || "",
				wan_ip: info[9]["ipv4-address"]?.[0]?.address || "",
			};
