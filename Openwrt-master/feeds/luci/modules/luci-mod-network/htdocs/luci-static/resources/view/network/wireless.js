...
var CBIWifiFrequencyValue = form.Value.extend({
	...
	load: function (section_id) {
		return Promise.all([
			...
			this.channels = {
				...
				'5g_160MHz': L.hasSystemFeature('hostapd', 'acs') ? ['auto', 'auto', true] : [],
				'6g': [],
				'60g': []
			};
			...
			for (var i = 0; i < data[1].length; i++) {
				...
				if (data[1][i].mhz >= 2412 && data[1][i].mhz <= 2484)
					band = '2g';
				else if (data[1][i].mhz >= 5160 && data[1][i].mhz <= 5650)
					band = '5g_160MHz';
				else if (data[1][i].mhz > 5650 && data[1][i].mhz <= 5885)
					band = '5g';
			...
			this.htmodes = {
				'': ['', '20 MHz', true],
				'n_chan165': ['HT20', '20 MHz', htmodelist.HT20],
				'ac_chan165': ['VHT20', '20 MHz', htmodelist.VHT20],
				'ax_chan165': ['HE20', '20 MHz', htmodelist.HE20],
				'n': [
	...
	toggleWifiHTMode: function (elem) {
		...
		if (165 == chan.value) {
			this.setValues(bwdt, this.htmodes[mode.value.concat("_chan165")]);
		}
		else if (mode.value == 'ac' || mode.value == 'ax') {
			if (chan.value > 128) {
				this.setValues(bwdt, this.htmodes[mode.value.concat("_over128")]);
			}
			else
				this.setValues(bwdt, this.htmodes[mode.value.concat("_below128")]);
		}
		else {
	...
	toggleWifiChannel: function (elem) {
		...
		var chan = elem.querySelector('.channel');
		var bwdt = elem.querySelector('.htmode');
		if (bwdt.value == 'HE160' || bwdt.value == 'VHT160') {
			this.setValues(chan, this.channels['5g_160MHz']);
		}
		else
...
return view.extend({
	...
	load: function () {
		// Compatible with situations where easymesh configuration file does not exist
		const easymeshPromise = new Promise(resolve => {
			uci.load('easymesh').then(() => resolve()).catch(() => resolve());
		});

		return Promise.all([
			uci.changes(),
			uci.load('wireless'),
			fs.exec('/sbin/power_level', ['1', '', '', '']),
			easymeshPromise,
		]);
	},
		...
		s.addModalOptions = function (s) {
			return network.getWifiNetwork(s.section).then(function (radioNet) {
				...
				if (hwtype == 'mac80211') {
					...
					o = ss.taboption('general', form.ListValue, 'power_level', _('Power Level'), _('High - Calibration value; Middle - Calibration value -3db; Low - Calibration value -6db'));
					o.value('0', _('Low'));
					o.value('1', _('Middle'));
					o.value('2', _('High'));
					o.write = function (section_id, value) {
						var band = uci.get('wireless', section_id, 'band');
						fs.exec('/sbin/power_level', ['0', String(value), band, section_id]);
					};
				...
				if (uci.get("easymesh", "status", "enable") == "1") {
					o.readonly = true
				}
