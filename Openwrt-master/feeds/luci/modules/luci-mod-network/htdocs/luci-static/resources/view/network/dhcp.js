...
return view.extend({
	...
	render: function(hosts_duids_pools) {
		...
		o = s.taboption('advanced', form.Flag, 'internal_server',
		_('Enable internal network server'),
		_('Resolve DNS requests using only internal servers to access internal addresses.'));
		o.optional = true;
		o.write = o.remove = function(section, value) {
			if(value == null){
				value = 0;
			}
			fs.exec('/sbin/internal_server', [section, value]);
			if(value == 1){
				uci.set('dhcp', section, 'rebind_protection', '0')
				uci.unset('dhcp', section, 'rebind_localhost')
			}
		};
