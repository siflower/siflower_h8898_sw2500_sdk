...
return view.extend({
	load: function() {
		var tasks = [
			...
			L.resolveDefault(fs.stat('/sbin/ota.sh'), {}),
	...
	handleBlockWrite: function(hostname, mtdblocks, ev) {
		var mtdblock = dom.parent(ev.target, '.cbi-section').querySelector('[data-name="mtdselect"] select').value;
		var mtdname = mtdblocks[(mtdblock*2+1)];
		if (mtdname != "factory") {
			alert(_('Only factory partition can be write! Please select factory first.'));
			return;
		}
		return ui.uploadFile('/tmp/mtdblock.bin', ev.target.firstChild)
			.then(L.bind(function(btn, res) {
				btn.firstChild.data = _('Erasing...');
				return fs.exec('/sbin/mtd', [ 'erase', mtdname ]);
			}, this, ev.target))
			.then(L.bind(function(btn, res) {
				return fs.exec('/sbin/mtd', [ 'write', '/tmp/mtdblock.bin', mtdname ]);
			}, this, ev.target))
			.catch(function(e) { ui.addNotification(null, E('p', e.message)) })
			.finally(L.bind(function(btn, input) {
				btn.firstChild.data = _('Flash mtdblock');
			}, this, ev.target));
	},
	...
	updateConfig: async function() {
        var otaPromise = uci.callLoad('ota');

        try {
            var PromiseResult = await Promise.all([otaPromise]);
            ...
            var detectionResult = await new Promise(function(resolve, reject) {
                ...
                var intervalId = setInterval(async function() {
                    if(status == 'null') {
                        ...
                        var otaPromise = uci.callLoad('ota');
                        var promiseList = [otaPromise]
                        var PromiseResult = await Promise.all(promiseList);
                        var result = PromiseResult[0]['ota'][status]
                        if (result == flag) {
                            clearInterval(intervalId);
	...
	handleOTAupgrade: async function() {
		if (!confirm(_('Are you sure you want to upgrade online? The upgrade process may take three to five minutes, please ensure that the machine will not be powered off.')))
			return;

		ui.showModal(_('Checking Updates…'), [
			E('span', { 'class': 'spinning' }, _('Check if updates are needed.'))
		]);

		fs.exec('/sbin/ota.sh',[ 'check_updates' ])

		var need_ota = await this.timerWaitStatus(5, 'need_ota', '1');

		var PromiseResult = await this.updateConfig();
		console.log(PromiseResult)
		if(need_ota) {
			var body = [];
			body.push(E('p', _("New version detected, choose whether to download or not")));
	
			var downloadbtn = E('button', {
				'class': 'btn cbi-button-action important',
				'click': ui.createHandlerFn(this, 'handleOTAdownload'),
			}, [ _('Download') ]);
			...
			body.push(E('div', { 'class': 'right' }, [
				...
				}, [ _('Cancel') ]), ' ', downloadbtn
			]));
			ui.showModal(_('Download firmware?'), body);
		} else {
			var body = [];
			body.push(E('p', _("The current version is already the latest and does not require any updates")));
			body.push(E('div', { 'class': 'right' }, [
				E('button', {
					'class': 'btn',
					'click': ui.createHandlerFn(this, function(ev) {
			...
			ui.showModal(_('No need to upgrade'), body);
		}
	},

	handleOTAdownload: async function() {
		fs.exec('/sbin/ota.sh',[ 'download_firmware' ])

		ui.showModal(_('Downloading Firmware…'), [
			E('span', { 'class': 'spinning' }, _('Firmware download in progress, please wait patiently for three minutes and do not cut off the power supply'))
		]);

		var check_file = await this.timerWaitStatus(120, 'check_file', '1');

		if(check_file) {
			ui.showModal(_('Upgrading Firmware…'), [
				E('span', { 'class': 'spinning' }, _('The latest firmware has been downloaded and is currently being upgraded... Please refresh the page in two minutes'))
			]);
			fs.exec('/sbin/ota.sh',[ 'do_upgrade' ])
		} else {
			var PromiseResult = await this.updateConfig();
			var download_res = PromiseResult[0]["ota"].download_res
			if(download_res) {
				var body = [];
				body.push(E('p', _("There was an error during the download process, please try again later")));
				body.push(E('div', { 'class': 'right' }, [
					E('button', {
						'class': 'btn',
						'click': ui.createHandlerFn(this, function(ev) {
				...
				ui.showModal(_('Download error'), body);
			} else {
				var body = [];
				body.push(E('p', _("Download failed, please check the network status and try again")));
				body.push(E('div', { 'class': 'right' }, [
					E('button', {
						'class': 'btn',
						'click': ui.createHandlerFn(this, function(ev) {
				...
				ui.showModal(_('Download fail'), body);
			}
		}
	},
	...
	render: function(rpc_replies) {
		console.log(rpc_replies)
		var has_sysupgrade = (rpc_replies[0].type == 'file'),
			has_otaupgrade = (rpc_replies[1].type == 'file'),
		    hostname = rpc_replies[2],
		    procmtd = rpc_replies[3],
		    procpart = rpc_replies[4],
		    procmounts = rpc_replies[5],
			otaconfig = rpc_replies[6],
		...
		if (mtdblocks.length) {
			...
			o = ss.option(form.Button, 'mtdrestore', _('Restore mtdblock'));
			o.inputstyle = 'action important';
			o.inputtitle = _('Flash mtdblock');
			o.onclick = L.bind(this.handleBlockWrite, this, hostname, mtdblocks);
		}
		...
		o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('OTA upgrade'),
			has_otaupgrade
				? _('Check if the latest OTA image exists and choose whether to update it.')
				: _('Sorry, there is no OTA support present.'));

		ss = o.subsection;

		if (has_otaupgrade) {
			o = ss.option(form.Button, 'otaupgrade', _('Image'));
			o.inputstyle = 'action important';
			o.inputtitle = _('OTA online...');
			o.onclick = L.bind(this.handleOTAupgrade, this, storage_size, has_rootfs_data);

			o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Auto_ota'),
			_('Whether to allow silent OTA upgrades every Monday from 2:00am to 5:00am.'));

			ss = o.subsection;

			o = ss.option(form.Flag, 'auto_ota', _('Enable'));
			o.optional = true;
			o.write = o.remove = function(section, value) {
				if(value == null){
					value = 0;
				}
				if(value == 1){
					uci.callSet('ota', 'auto_ota', { 'enable': 1 });
					return true
				} else {
					uci.callSet('ota', 'auto_ota', { 'enable': 0 });
					return false
				}
			};
			o.load = function(section_id) {
				var enable = otaconfig['auto_ota'].enable;
				console.log(enable)
				if(enable == 1){
					return true
				} else {
