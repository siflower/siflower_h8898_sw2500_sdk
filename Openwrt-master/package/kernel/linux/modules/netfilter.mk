...
define KernelPackage/nf-flow-ipv4
  SUBMENU:=$(NF_MENU)
  HIDDEN:=1
  TITLE:=Netfilter IPv4 flowtable support (not used by firewall4)
  KCONFIG:=CONFIG_NF_FLOW_TABLE_IPV4
  DEPENDS:=+kmod-nf-flow
  FILES:=$(LINUX_DIR)/net/ipv4/netfilter/nf_flow_table_ipv4.ko
  AUTOLOAD:=$(call AutoProbe,nf_flow_table_ipv4)
endef

$(eval $(call KernelPackage,nf-flow-ipv4))


define KernelPackage/nf-flow-ipv6
  SUBMENU:=$(NF_MENU)
  HIDDEN:=1
  TITLE:=Netfilter IPv6 flowtable support (not used by firewall4)
  KCONFIG:=CONFIG_NF_FLOW_TABLE_IPV6
  DEPENDS:=@IPV6 +kmod-nf-flow
  FILES:=$(LINUX_DIR)/net/ipv6/netfilter/nf_flow_table_ipv6.ko
  AUTOLOAD:=$(call AutoProbe,nf_flow_table_ipv6)
endef

$(eval $(call KernelPackage,nf-flow-ipv6))
...
define KernelPackage/nft-core
  ...
  AUTOLOAD:=$(call AutoLoad,49,$(notdir $(NFT_CORE-m)))
  KCONFIG:= \
	CONFIG_NFT_COMPAT=n \
	CONFIG_NFT_QUEUE=n \
	$(KCONFIG_NFT_CORE)
...
define KernelPackage/nft-hash
  SUBMENU:=$(NF_MENU)
  TITLE:=Netfilter nf_tables hash support
  DEPENDS:=+kmod-nft-core
  FILES:=$(foreach mod,$(NFT_HASH-m),$(LINUX_DIR)/net/$(mod).ko)
  AUTOLOAD:=$(call AutoProbe,$(notdir $(NFT_HASH-m)))
  KCONFIG:=$(KCONFIG_NFT_HASH)
endef

$(eval $(call KernelPackage,nft-hash))


define KernelPackage/nft-numgen
  SUBMENU:=$(NF_MENU)
  TITLE:=Netfilter nf_tables numgen support
  DEPENDS:=+kmod-nft-core
  FILES:=$(foreach mod,$(NFT_NUMGEN-m),$(LINUX_DIR)/net/$(mod).ko)
  AUTOLOAD:=$(call AutoProbe,$(notdir $(NFT_NUMGEN-m)))
  KCONFIG:=$(KCONFIG_NFT_NUMGEN)
endef

$(eval $(call KernelPackage,nft-numgen))


define KernelPackage/nft-objref
  SUBMENU:=$(NF_MENU)
  TITLE:=Netfilter nf_tables objref support
  DEPENDS:=+kmod-nft-core
  FILES:=$(foreach mod,$(NFT_OBJREF-m),$(LINUX_DIR)/net/$(mod).ko)
...
define KernelPackage/nft-quota
  SUBMENU:=$(NF_MENU)
  TITLE:=Netfilter nf_tables quota support
  DEPENDS:=+kmod-nft-core
  FILES:=$(foreach mod,$(NFT_QUOTA-m),$(LINUX_DIR)/net/$(mod).ko)
  AUTOLOAD:=$(call AutoProbe,$(notdir $(NFT_QUOTA-m)))
  KCONFIG:=$(KCONFIG_NFT_QUOTA)
endef

$(eval $(call KernelPackage,nft-quota))
