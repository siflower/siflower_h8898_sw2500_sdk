						 ...
						 struct sigma_cmd *cmd)
{
    ...
    printf("[sigma run]cmd_ap_set_wireless\n");
					   ...
					   struct sigma_cmd *cmd)
{
    ...
    printf("[SIGMA]cmd_ap_config_commit, ap_mode %d type %d\n", mode, drv);
...
write_conf:
    ...
    printf("[SIGMA]open file success\n");

	ifname = get_hostapd_ifname(dut);
    printf("[SIGMA]get ifname %s\n", ifname);
 ...
 * [AP channel numbers]
 */
//'{"addr":"92:49:b7:c5:51:f9","mode":0,"op_class":81,"channel":149,"duration":100,"bssid":"10:16:88:1A:A1:B4","ssid":"String"}'
						  ...
						  struct sigma_cmd *cmd)
{
	...
	if (strcasecmp(param, "SSID") == 0) {
		...
		if (!val)
			return INVALID_SEND_STATUS;
		snprintf(resp, sizeof(resp), "GET_PMK %s", val);
...
#define IFACE "default_radio"
#define RADIO "radio"

int g_wlan = 0; //0 for wlan0 5g; 1 for wlan1 2.4g
char g_wlan_mac[32] = "10:16:88:c1:0b:c4";
#define WIFI_BASE_ADDR (g_wlan == 0 ? 0xa3000000 : 0xa4000000)
#define REG_MAC_CORE_BASE_ADDR   (WIFI_BASE_ADDR+0x00B00000)

//test device default macaddr
char mac_24g[] = "10:16:88:c1:0b:c0";
char mac_5g[] = "10:16:88:c1:0b:c4";

void get_cmd_result(char *cmd, char *buf)
{
    FILE *f;
    f = popen(cmd, "r");
    if(!f){
...
void uci_set(unsigned int band, const char *section, const char *option, char *val)
{
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "uci set wireless.%s%d.%s=%s", section, band, option, val);
    printf("[siflower sigma]%s\n", cmd);
    system(cmd);
}

void uci_get(unsigned int band, const char *section, const char *option, char *buf)
{
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "uci get wireless.%s%d.%s", section, band, option);
    get_cmd_result(cmd, buf);
}
...
//8.2
static enum sigma_cmd_result siflower_cmd_ap_ca_version(struct sigma_dut *dut,
					       struct sigma_conn *conn,
					       struct sigma_cmd *cmd)
{
	send_resp(dut, conn, SIGMA_COMPLETE, "version,1.0");
...
//8.3
int bcc_set = 0;
static enum sigma_cmd_result siflower_cmd_ap_config_commit(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    char tmpcmd[128];
    printf("[siflower sigma]siflower_cmd_ap_config_commit\n");
    uci_commit();
    //bcc和20MHz是一起配的，但是20MHz要重启后才生效，如果bcc立即生效则很可能出现80M跑bcc从而出现phy error
    //最稳妥的做法就是等wifi起来之后再配bcc
    if(bcc_set){
        bcc_set = 0;
        snprintf(tmpcmd, sizeof(tmpcmd), "echo 2 1 > /sys/kernel/debug/ieee80211/phy%d/siwifi/wfa_setting", g_wlan);
        system(tmpcmd);
        printf("set BCC finish\n");
    }

    //MBO字段，reset的时候应该默认mbo_assoc_disallow功能停用，需要的时候再使能
    snprintf(tmpcmd, sizeof(tmpcmd), "hostapd_cli -i wlan%d set mbo_assoc_disallow 0", g_wlan);
    system(tmpcmd);
...
//8.4
static enum sigma_cmd_result siflower_cmd_ap_deauth_sta(struct sigma_dut *dut,
					       struct sigma_conn *conn,
					       struct sigma_cmd *cmd)
{
    const char *val;
	...
	val = get_param(cmd, "sta_mac_address");
	if (val) {
        snprintf(buf, sizeof(buf), "wpa_cli -g var/run/hostapd/wlan%d deauthenticate %s", g_wlan, val);
        printf("ap_deauth_sta run: %s\n", buf);
        system(buf);
    }
    return SUCCESS_SEND_STATUS;
}
...
//8.5
static enum sigma_cmd_result siflower_cmd_ap_get_mac_address(struct sigma_dut *dut,
						    struct sigma_conn *conn,
						    struct sigma_cmd *cmd)
{
	/* const char *name = get_param(cmd, "NAME"); */
	...
	if (val) {
		if (strcasecmp(val, "5G") == 0)
            ...
            snprintf(ifname, sizeof(ifname), "wlan1");
    } else {
        if(g_wlan == 1)
            snprintf(ifname, sizeof(ifname), "wlan1");
        else
    ...
    printf("siflower_cmd_ap_get_mac_address %s\n", ifname);

    snprintf(run_cmd, sizeof(run_cmd), "ifconfig %s | head -n 1 |awk '{print $NF}' | tr -d '\n'", ifname);
    f = popen(run_cmd, "r");
    if(!f){
        printf("error get mac by ifconfig\n");
    } else {
        while (fgets(addr, sizeof(addr), f) != NULL) {}
        pclose(f);
    }
    printf("get addr %s\n", addr);
    ...
    if(strlen(addr)  < 15){
        if(g_wlan == 1) {
            snprintf(resp, sizeof(resp), "mac,%s", mac_5g);
            sprintf(g_wlan_mac, "%s", mac_5g);
        } else {
            snprintf(resp, sizeof(resp), "mac,%s", mac_24g);
            sprintf(g_wlan_mac, "%s", mac_24g);
        }
    } else {
        snprintf(resp, sizeof(resp), "mac,%s", addr);
        sprintf(g_wlan_mac, "%s", addr);
    }
...
//8.6
static enum sigma_cmd_result siflower_cmd_ap_get_parameter(struct sigma_dut *dut,
						  struct sigma_conn *conn,
						  struct sigma_cmd *cmd)
{
	char value[64], resp[512];

	const char *param = get_param(cmd, "parameter");

	if (strcasecmp(param, "SSID") == 0) {
        uci_get(g_wlan, IFACE, "ssid", value);
		snprintf(resp, sizeof(resp), "SSID,%s", value);
	} else if (strcasecmp(param, "PSK") == 0) {
        uci_get(g_wlan, IFACE, "ssid", value);
		snprintf(resp, sizeof(resp), "PSK,%s", value);
...
//8.7
static enum sigma_cmd_result siflower_cmd_ap_preset_testparameters(struct sigma_dut *dut,
                         struct sigma_conn *conn,
			             struct sigma_cmd *cmd)
{
    return 0;
...
//当ENABLE_REG置于1时，VALUE_REG的bit0-7为ltf值，bit8-15为gi值。需要配合对应的lmac镜像使用
//目前看来GI节点已满足要求，若今后需要时再修改LMCA，此处仅做记录，lmac尽量不动
#define WFA_GI_LTF_SET_VALUE_REG    0xc1e00070
#define WFA_GI_LTF_SET_ENABLE_REG   0xc1e00074

//当此寄存器为1时，lmac中phyctrlinfo1将总会使用bcc。需要配合lmac使用
//目前只在mu中会用到，且有节点可以尝试配置。因此处仅做记录，lmac尽量不动。若测试一直不通过则再使用此方法
#define WFA_BCC_EN_REG      0xc1e00078
//8.8
void btm_reset(void);
static enum sigma_cmd_result siflower_cmd_ap_reset_default(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    //特定的改BCC或者停beacon的case再开启
    //system("echo 0 0 > /sys/kernel/debug/ieee80211/phy0/siwifi/wfa_setting");
    //system("echo 0 0 > /sys/kernel/debug/ieee80211/phy1/siwifi/wfa_setting");

    btm_reset();
    printf("[siflower sigma]siflower_cmd_ap_reset_default\n");
    system("rm -f /etc/config/wireless;wifi config");
    usleep(500000);
...
//8.14
static enum sigma_cmd_result siflower_cmd_ap_set_11d(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    const char *val;
	val = get_param(cmd, "CountryCode");
	if (val) {
        uci_set(g_wlan, RADIO, "country", (char *)val);
        return SUCCESS_SEND_STATUS;
    } else {
        sigma_dut_print(dut, DUT_MSG_ERROR, "Failed to get CountryCode");
	    return INVALID_SEND_STATUS;
    }
}

//8.15
static enum sigma_cmd_result siflower_cmd_ap_set_11h(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    const char *val;
	...
	if (val) {
		if (strcasecmp(val, "Disable") == 0)
            uci_set(g_wlan, RADIO, "rd_disabled", "1");
        else
            uci_set(g_wlan, RADIO, "rd_disabled", "0");
    }

	val = get_param(cmd, "dfs_chan");
	if (val) {
        uci_set(g_wlan, RADIO, "channel", (char *)val);
    }
...
//8.16
//11n应该用不上了，工具都测11ax
static enum sigma_cmd_result siflower_cmd_ap_set_11n_wireless(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    const char *val;
	...
	if (val) {
        if (strcasecmp(val, "1SS") == 0)
            ...
            sigma_dut_print(dut, DUT_MSG_ERROR, "Failed to get Spatial_Rx_Stream: %s\n", val);
    }
	...
	if (val) {
        ...
        //get HE HT or VHT
        char ht_mode[8];
        snprintf(run_cmd, sizeof(run_cmd), "uci get wireless.radio%d.htmode | tr -d [0-9+]", g_wlan);
        get_cmd_result(run_cmd, ht_mode);
        if(strlen(ht_mode) < 2){
            sprintf(ht_mode, "HE");
        }
        if(strcasecmp(val, "auto") == 0) {
            ...
            width = atoi(val) == 20 ? 20 : 40;
        }
        snprintf(buf, sizeof(buf), "%s%d", ht_mode, width);
        uci_set(g_wlan, RADIO, "htmode", buf);
    }

    return SUCCESS_SEND_STATUS;
}

#define AIFSN_OFFSET    0
#define AIFSN_MASK      0x0000000F
#define CWMIN_OFFSET    4
#define CWMIN_MASK      0x000000F0
#define CWMAX_OFFSET    8
#define CWMAX_MASK      0x00000F00
#define TXOP_OFFSET     12
#define TXOP_MASK       0x0FFFF000
#define NXMAC_EDCA_BASE_ADDR    (REG_MAC_CORE_BASE_ADDR + 0x00000200)
static int siflower_parse_qos_params(unsigned int ac, const char *acm, const char *cwmin, const char *cwmax,
                            const char *aifs, const char *txop, struct sigma_dut *dut, struct sigma_conn *conn)
{
	int val;
    unsigned int reg;
    int edca_val = 0;
	...
	if (cwmin) {
		...
		edca_val |= ((val << CWMIN_OFFSET) & CWMIN_MASK);
	}
	...
	if (cwmax) {
		...
		edca_val |= ((val << CWMAX_OFFSET) & CWMAX_MASK);
	}
	...
	if (aifs) {
		...
		edca_val |= ((val << AIFSN_OFFSET) & AIFSN_MASK);
	}
	...
	if (txop) {
		...
		edca_val |= ((val << TXOP_OFFSET) & TXOP_MASK);
	}

    reg = NXMAC_EDCA_BASE_ADDR + 0x4 * ac;
    printf("set EDCA %d: reg %08x, val %08x\n", ac, reg, (unsigned int)edca_val);
    sprintf(buf, "devmem 0x%08x 32 0x%08x", reg, (unsigned int)edca_val);
    printf("cmd: %s\n", buf);
...
//8.17
static enum sigma_cmd_result siflower_cmd_ap_set_apqos(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    const char *val;
	...
	val = get_param(cmd, "ACM_BK");
	if (val) {
        ret |= siflower_parse_qos_params(AP_AC_BK, val, get_param(cmd, "cwmin_BK"), get_param(cmd, "cwmax_BK"),
                                get_param(cmd, "AIFS_BK"),get_param(cmd, "TXOP_BK"), dut, conn);
    }

	val = get_param(cmd, "ACM_BE");
	if (val) {
        ret |= siflower_parse_qos_params(AP_AC_BE, val, get_param(cmd, "cwmin_BE"), get_param(cmd, "cwmax_BE"),
                                get_param(cmd, "AIFS_BE"),get_param(cmd, "TXOP_BE"), dut, conn);
    }

	val = get_param(cmd, "ACM_VI");
	if (val) {
        ret |= siflower_parse_qos_params(AP_AC_VI, val, get_param(cmd, "cwmin_VI"), get_param(cmd, "cwmax_VI"),
                                get_param(cmd, "AIFS_VI"),get_param(cmd, "TXOP_VI"), dut, conn);
    }

	val = get_param(cmd, "ACM_VO");
	if (val) {
        ret |= siflower_parse_qos_params(AP_AC_VO, val, get_param(cmd, "cwmin_VO"), get_param(cmd, "cwmax_VO"),
                                get_param(cmd, "AIFS_VO"),get_param(cmd, "TXOP_VO"), dut, conn);
    }
...
//8.20
static enum sigma_cmd_result siflower_cmd_ap_set_pmf(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    const char *val;
    ...
    if(val){
        if (strcasecmp(val, "Required") == 0)
            uci_set(band, IFACE, "ieee80211w", "2");
        else if(strcasecmp(val, "Optiona") == 0)
            uci_set(band, IFACE, "ieee80211w", "1");
        else if(strcasecmp(val, "Disabled") == 0)
            uci_set(band, IFACE, "ieee80211w", "0");
        else
            printf("siflower_cmd_ap_set_pmf get pmf %s error\n", val);
    }
    return SUCCESS_SEND_STATUS;
}
...
struct btm btm_list[5];

//用数组暂存neighbor bssid信息，添加时往最近的一个空位添加，如果遇到mac地址一样的，就更新信息而不是添加信息
void update_btm(char *str)
{
    printf("update_btm %s\n", str);
    for (int i = 0; i < 5; i++) {
        if(btm_list[i].enable == 0){
            sprintf(btm_list[i].str, "%s", str);
            btm_list[i].enable = 1;
            printf("add %d %s\n", i, btm_list[i].str);
            break;
        } else if(strncasecmp(btm_list[i].str, str, 28) == 0){
            sprintf(btm_list[i].str, "%s", str);
            btm_list[i].enable = 1;
            printf("update %d %s\n", i, btm_list[i].str);
            break;
...
{
    for (int i = 0; i < 5; i++) {
        if(btm_list[i].enable == 1){
            printf("%d: %s\n", i, btm_list[i].str);
        }
    }
}

//workround方法，往/tmp/case里面写值，现需要5和8，表明当前正在测MBO case5或者8，需要一些额外手段
int BTM_DisAssoc_Imnt = 0;
int Term_Bit = 0;
int Term_Duration = 0;
int Term_TSF = 0;
//4.2.8
...
int btm_case8_if_change_neighbor(int index)
{
    //workround：在case8中，第三个neighbor（index=2）的Pref需要在第一次发btm的时候变为0
    if(BTM_case8 == 1){
        if(index == 2)
            return 1;
        else
            return 0;
    //workround：在case8中，第1,2个neighbor（index=0,1）的Pref需要在第二次发btm的时候变为0
    //第二次发送时BTM_case8=2
    } else {
        if(index == 0 || index == 1)
            return 1;
...
void btm_add_neighbor(struct sigma_dut *dut, char *cmd_buf){
    char buf[256] = "";
    unsigned char m[6];
    int add_default = 1;
    int i;
    ...
    if(BTM_case8) {
        for (i = 0; i < 5; i++) {
            if(btm_list[i].enable == 1){
                add_default = 0;
                if(btm_case8_if_change_neighbor(i)){
                    sprintf(case8, "%s", btm_list[i].str);
                    len = strlen(case8);
                    case8[len - 2] = str0[0];
                    case8[len - 1] = str0[1];
                    strcat(buf, case8);
                    printf("btm case 8-%d en %d: %s\n", BTM_case8, i, case8);
                } else {
                    strcat(buf, btm_list[i].str);
                    printf("btm case 8-%d en %d: %s\n", BTM_case8, i, btm_list[i].str);
                }
            }
        }
        //BTM_case8的在发送一次后1变2,2变1
        BTM_case8 = 3 - BTM_case8;
    } else if(BTM_case5_set){
        //根据对方提供的抓包，发现在case5的4.2.5.3中，将BTMReq_DisAssoc_Imnt配成1之后的BTM request，第一个neighbor信息（也就是自己）的最后两位Nebor_Pref应该变成00
        //因此这个条件只针对于4.2.5.3，在case5使能且BTMReq_DisAssoc_Imnt配成1之后才会启用
        for (i = 0; i < 5; i++) {
            if(btm_list[i].enable == 1){
                add_default = 0;
                if(i == 0){
                    snprintf(case8, sizeof(case8), "%s", btm_list[i].str);
                    len = strlen(case8);
                    case8[len - 2] = str0[0];
                    case8[len - 1] = str0[1];
                    strcat(buf, case8);
                    printf("btm case 5-3 en %d: %s\n", i, case8);
                } else {
                    strcat(buf, btm_list[i].str);
                    printf("btm case 5-3 en %d: %s\n", i, btm_list[i].str);
                }
    ...
    //当没有任何neighbor信息时，发BTM默认应该添加ap自己macaddr的信息(4.2.5.4)
    if(add_default){
        if(parse_mac_address(dut, g_wlan_mac, m) < 0)
            sprintf(buf, " neighbor=10:16:88:49:f6:c5,23,115,36,0,0603000000030100");
        else
            sprintf(buf, " neighbor=%02x:%02x:%02x:%02x:%02x:%02x,23,115,36,0,0603000000030100",m[0],m[1],m[2],m[3],m[4],m[5]);
        printf("btm add default %s\n", buf);
    }
    printf("btm_add_neighbor: %s\n", buf);
    strcat(cmd_buf, buf);
}
...
void btm_reset(void)
{
    ...
    BTM_case5_set = 0;
    BTM_bcn_down = 0;
    system("devmem 0xc1e0008c 32 0x0");
    get_cmd_result(cmd, value);
    if(strncasecmp(value, "8", 1) == 0) {
        BTM_case8 = 1;
        printf("now into case 8 mode\n");
    } else if(strncasecmp(value, "5", 1) == 0){
        BTM_case5 = 1;
        printf("now into case 5-3 mode\n");
    }
}

//MBO 4.2.5
static enum sigma_cmd_result siflower_cmd_ap_set_rfeature(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
    const char *val;
    ...
    int btm_class = 115, btn_ch = 36, btm_pref = 0;

#if 0
    val = get_param(cmd, "LTF");
    if (val) {
        if_set = 1;
        if (strcasecmp(val, "3.2") == 0)
            ltf = 0;
        else if(strcasecmp(val, "6.4") == 0)
            ltf = 1;
        else if(strcasecmp(val, "12.8") == 0)
            ltf = 2;
    }
#endif
    val = get_param(cmd, "GI");
    if (val) {
        if (strcasecmp(val, "0.8") == 0)
            gi = 0;
        else if(strcasecmp(val, "1.6") == 0)
            gi = 1;
        else if(strcasecmp(val, "3.2") == 0)
            gi = 2;

        snprintf(tmpcmd, sizeof(tmpcmd), "echo %d > /sys/kernel/debug/ieee80211/phy%d/siwifi/dl_ofdma/gi", gi, g_wlan);
        system(tmpcmd);
    }
	...
	if (val) {
        tmp = atoi(val);
        if((tmp == 20)||(tmp == 40)||(tmp == 80)||(tmp == 160)) {
            snprintf(buf, sizeof(buf), "HE%s", val);
            uci_set(g_wlan, RADIO, "htmode", buf);
        } else {
            printf("input width err: %s\n", val);
        }
        system("uci commit wireless;wifi");
        usleep(3000000);
    }
	...
	val = get_param(cmd, "Nebor_BSSID");
	if (val) {
        if(parse_mac_address(dut, val, m) < 0)
            return -1;
        //mac, bssid info(00000000), op, ch, phy type(00), ID+len(0301), pref
        sprintf(buf, " neighbor=%02x:%02x:%02x:%02x:%02x:%02x,23,%d,%d,0,06030000000301%02x", m[0], m[1], m[2], m[3], m[4], m[5], btm_class, btn_ch, btm_pref);
        sprintf(set_nb, "hostapd_cli -i wlan%d set_neighbor %02x:%02x:%02x:%02x:%02x:%02x ssid=342e322e352e31 nr=%02x%02x%02x%02x%02x%02x17000000%02x%02x0006030000000301%02x",
                g_wlan, m[0], m[1], m[2], m[3], m[4], m[5], m[0], m[1], m[2], m[3], m[4], m[5], btm_class, btn_ch, btm_pref);
        printf(set_nb);
        printf("\n");
        system(set_nb);
        update_btm(buf);
	...
	if (val){
        if (strcasecmp(val, "Enable") == 0)
            uci_set(g_wlan, IFACE, "mbo", "1");
        else
            uci_set(g_wlan, IFACE, "mbo", "0");
        system("uci commit wireless;wifi");
        usleep(3000000);
    }
	...
	val = get_param(cmd, "BSS_Term_Duration");
	if (val){
        Term_Duration = atoi(val);
    }

	val = get_param(cmd, "BSS_Term_TSF");
	if (val){
        //只有4.2.5.4会配TSF，配完发btm的时候把beacon停掉
        BTM_bcn_down = 1;
        Term_TSF = atoi(val);
    }
...
//8.24
static enum sigma_cmd_result siflower_cmd_ap_set_security(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
	const char *val;
    ...
    printf("[siflower sigma]ap_set_security\n");

	val = get_param(cmd, "Encrypt");
	if (val) {
        set_encrypt = 1;
		if (strcasecmp(val, "TKIP") == 0){
            sprintf(encrypt, "+tkip");
        } else if(strcasecmp(val, "AES") == 0){
            sprintf(encrypt, "+aes");
        //not support gcmp
        //} else if(strcasecmp(val, "AES-GCMP") == 0){
        } else {
		    send_resp(dut, conn, SIGMA_INVALID, "ErrorCode,Unsupported MODE %s Not Supported");
            return 0;
        }
    }
	...
	if (val) {
        ...
        } else if(strcasecmp(val, "WPA3-PSK") == 0){
            snprintf(buf, sizeof(buf), "sae");
        } else if(strcasecmp(val, "WPA2-PSK") == 0){
            snprintf(buf, sizeof(buf), "psk2");
        } else if(strcasecmp(val, "WPA2-PSK-Mixed") == 0){
            snprintf(buf, sizeof(buf), "psk-mixed");
        } else if(strcasecmp(val, "WPA2-PSK-SAE") == 0){
            snprintf(buf, sizeof(buf), "sae-mixed");
        } else if(strcasecmp(val, "SAE") == 0){
            //说是sae，但是测试case里面提到了psk字段，有可能需要的是mixed
            //snprintf(buf, sizeof(buf), "sae-mixed");
            snprintf(buf, sizeof(buf), "sae");
            //4.7 如果不支持wpa2 ent的全部换成psk2
        } else if(strcasecmp(val, "WPA2-Ent") == 0){
            snprintf(buf, sizeof(buf), "psk2");
        } else {
		    send_resp(dut, conn, SIGMA_INVALID, "ErrorCode,Unsupported MODE, Not Supported");
            return 0;
        }
        sprintf(KeyMgnt, "%s%s", buf, encrypt);
        uci_set(g_wlan, IFACE, "encryption", KeyMgnt);
    } else {
        if(set_encrypt){
            snprintf(run_cmd, sizeof(run_cmd), "uci get wireless.default_radio%d.encryption | awk -F '+' '{print $1}'", g_wlan);
            get_cmd_result(run_cmd, buf);
            sprintf(KeyMgnt, "%s%s", buf, encrypt);
            uci_set(g_wlan, IFACE, "encryption", KeyMgnt);
        }
    }
	...
	if (val) {
        ...
        uci_set(g_wlan, IFACE, "key", buf);
	}

    val = get_param(cmd, "PMF");
    if(val){
        if (strcasecmp(val, "Required") == 0)
            uci_set(g_wlan, IFACE, "ieee80211w", "2");
        //手册上面写的Optiona，但正常单词应该是Optional？为避免它错了只比前几个字母就行
        else if(strncmp(val, "Optiona", 7) == 0)
            uci_set(g_wlan, IFACE, "ieee80211w", "1");
        else if(strcasecmp(val, "Disabled") == 0)
            uci_set(g_wlan, IFACE, "ieee80211w", "0");
        else
            printf("siflower_cmd_ap_set_pmf get pmf %s error\n", val);
    }
...
//8.26
static enum sigma_cmd_result siflower_cmd_ap_set_wireless(struct sigma_dut *dut,
						 struct sigma_conn *conn,
						 struct sigma_cmd *cmd)
{
	const char *val;
    ...
    printf("[siflower sigma]ap_set_wireless %s\n", ifname);
	...
	if (val) {
        ...
        snprintf(buf, sizeof(buf), "%s", val);
        uci_set(g_wlan, RADIO, "channel", buf);
	}

	val = get_param(cmd, "SSID");
	if (val) {
        snprintf(buf, sizeof(buf), "%s", val);
        uci_set(g_wlan, IFACE, "ssid", buf);
	}

	val = get_param(cmd, "Mode");
	if (val) {
        if(strcasecmp(val, "11ac") == 0){
            snprintf(mode, sizeof(mode), "VHT");
        } else if(strcasecmp(val, "11n") == 0){
            snprintf(mode, sizeof(mode), "HT");
        } else if(strcasecmp(val, "11ng") == 0){
            snprintf(mode, sizeof(mode), "HT");
        } else if(strcasecmp(val, "11ax") == 0){
	...
	if (val) {
        if(strcasecmp(val, "auto") == 0){
            //我们不支持auto选项，直接固定成20
            snprintf(buf, sizeof(buf), "%s20", mode);
            uci_set(g_wlan, RADIO, "htmode", buf);
        } else {
            tmp = atoi(val);
            if((tmp == 20)||(tmp == 40)||(tmp == 80)||(tmp == 160)) {
                snprintf(buf, sizeof(buf), "%s%s", mode, val);
                uci_set(g_wlan, RADIO, "htmode", buf);
            } else {
                printf("input width err: %s\n", val);
            }
	...
	if (val) {
        if (strcasecmp(val, "Disable") == 0)
            uci_set(g_wlan, RADIO, "ht_coex", "0");
        else
            uci_set(g_wlan, RADIO, "ht_coex", "1");
    }

#if 0
    //现在amsdu_threshold总是配为0
	val = get_param(cmd, "amsdu");
	if (val) {
        int amsdu_threshold;
        if (strcasecmp(val, "enable") == 0){
            ...
            amsdu_threshold = 1400;
        }
        snprintf(tmpcmd, sizeof(tmpcmd), "echo %d > /sys/kernel/debug/ieee80211/phy%d/siwifi/amsdu_threshold", amsdu_threshold, g_wlan);
        system(tmpcmd);
    }
#endif
    ...
    if (val) {
        if (strcasecmp(val, "Enable") == 0) {
            ...
            //system("devmem 0xc1e00078 32 0x0");
            snprintf(tmpcmd, sizeof(tmpcmd), "echo 2 0 > /sys/kernel/debug/ieee80211/phy%d/siwifi/wfa_setting", g_wlan);
            system(tmpcmd);
        }
    }
...
void ubus_cmd_add_name(char *name, char *buf)
{
    char str[32];
    sprintf(str, ", \"%s\":", name);
    strcat(buf, str);
...
void ubus_cmd_add_int(int n, char *buf)
{
    char str[16];
    sprintf(str, "%d", n);
    strcat(buf, str);
...
void ubus_cmd_add_str(char *value, char *buf)
{
    char str[256];
    sprintf(str, "%s", value);
    strcat(buf, str);
...
void btm_add_int(char *name, int n, char *buf)
{
    char str[256];
...
void btm_add_str(char *name, char *value, char *buf)
{
    char str[256];
...
struct bcn_req_val_mid {
    uint8_t BSSID[6];
    uint8_t SSID_len;
    uint8_t *SSID;
};

struct bcn_req_val_2 {
    uint32_t fixed_RepCon;   //default id:01, len:02, val:0000
    uint16_t fixed_RepDet_Head;
    uint8_t RepDet;
    uint8_t fixed_Request_ID; //id:0a
    uint32_t fixed_Request_Val; //len:03, val: 00 36 dd
    uint8_t fixed_IndReq[3]; //a4 01 01
};

char fixed_str_2[] = "01020000 0201 01 0a 030036dd a40101";

//对beacon request中SubElement ID: Request字段的解析
//例如：传入“0_54_221”得到报文中实际应该填入的“030036dd”，传入“0”得到“0100”
void sub_request_convert_to_hex_string(char *input, char *output)
{
    char input_copy[256];
    char *token;
    ...
    while (token != NULL && count < 4) {
        values[count++] = (unsigned char)atoi(token);
        token = strtok(NULL, "_");
    }

    sprintf(output, "0a%02x", count);
...
#if 1
//Dest_MAC,00:90:4c:81:ab:f5,RegClass,115,Channel,0,RandInt,50,MeaDur,20,MeaMode,active,BSSID,FF:FF:FF:FF:FF:FF,SSID,WiFi1-4.2.3_5G,RptCond,0,RptDet,1,MeaDurMand,0,APChanRpt,,ReqInfo,0_54_221,LastBeaconRptIndication,1
static int siflower_ap_send_frame_bcn_rpt_req(struct sigma_dut *dut,
					 struct sigma_cmd *cmd,
					 const char *ifname)
{
    const char *val;
    ...
    val = get_param(cmd, "Dest_MAC");
    if(val){
        if (parse_mac_address(dut, val, dest_mac) < 0) {
            sigma_dut_print(dut, DUT_MSG_ERROR,"MAC Address not in proper format");
            return -1;
        }
    } else {
        sigma_dut_print(dut, DUT_MSG_ERROR,"MAC Address not in proper format");
    ...
    val = get_param(cmd, "RegClass");
    if(val){
        u8val = atoi(val);
        op_class = u8val;
        snprintf(tmp,  sizeof(tmp), "%02x", u8val);
    ...
    val = get_param(cmd, "RandInt");
    if(val){
        u16val = atoi(val);
        snprintf(tmp,  sizeof(tmp), "%02x%02x", (u16val) & 0xff, (u16val >> 8) & 0xff);
        strlcat(buf, tmp, sizeof(buf));
    }

    val = get_param(cmd, "MeaDur");
    if(val){
        u16val = atoi(val);
        snprintf(tmp,  sizeof(tmp), "%02x%02x", (u16val) & 0xff, (u16val >> 8) & 0xff);
        strlcat(buf, tmp, sizeof(buf));
    ...
    val = get_param(cmd, "MeaMode");
    if(val){
        if (strcasecmp(val, "passive") == 0) {
            u8val = 0;
        } else if (strcasecmp(val, "active") == 0) {
            u8val = 1;
        } else if (strcasecmp(val, "table") == 0) {
            u8val = 2;
        } else {
            sigma_dut_print(dut, DUT_MSG_ERROR,
                    "MEA-MODE Value not correctly given");
    ...
    if(val){
        if (parse_mac_address(dut, val, bssid) < 0) {
            sigma_dut_print(dut, DUT_MSG_ERROR,"BSSID not in proper format");
            return -1;
        }
    ...
    val = get_param(cmd, "SSID");
    if(val){
	    snprintf(ssid_buf, sizeof(ssid_buf), "%s", val);
        ssid_len = strlen(ssid_buf);
        if(ssid_len > 0) {
            snprintf(tmp,  sizeof(tmp), "00%02x", ssid_len);
            strlcat(buf, tmp, sizeof(buf));
            for(int i = 0; i < ssid_len; i++){
                snprintf(tmp,  sizeof(tmp), "%02x", ssid_buf[i]);
                strlcat(buf, tmp, sizeof(buf));
            }
        }
    }

    val = get_param(cmd, "RptCond");
    if(val){
        u8val = atoi(val);
        snprintf(tmp,  sizeof(tmp), "0102%02x00", u8val);
        strlcat(buf, tmp, sizeof(buf));
    }

    val = get_param(cmd, "RptDet");
    if(val){
        u8val = atoi(val);
        snprintf(tmp,  sizeof(tmp), "0201%02x", u8val);
        strlcat(buf, tmp, sizeof(buf));
    }

    val = get_param(cmd, "ReqInfo");
    if(val){
        ...
        if(strlen(tmp) > 0){
            ...
            char output[16] = {0};
            //解析传入字段
            sub_request_convert_to_hex_string(tmp, output);
            strlcat(buf, output, sizeof(buf));
#else
            //以前是只要有值就固定发”0_54_221“对应的参数，后来发现它还会传”0“,因此还是做解析比较好
            snprintf(tmp,  sizeof(tmp), "0a030036dd");
            strlcat(buf, tmp, sizeof(buf));
#endif
        }
    }
    ...
    if(val){
        ...
        if(strlen(tmp) > 0){
            ...
            snprintf(tmp,  sizeof(tmp), "3303%02x%02x%02x", op_class, ap_chanrpt, ap_chanrpt_2);
            strlcat(buf, tmp, sizeof(buf));
        }
    }

    val = get_param(cmd, "LastBeaconRptIndication");
    if(val){
        u8val = atoi(val);
        snprintf(tmp,  sizeof(tmp), "a401%02x", u8val);
        strlcat(buf, tmp, sizeof(buf));
    }

    snprintf(cmd_buf, sizeof(cmd_buf), "hostapd_cli -i wlan%d req_beacon %02x:%02x:%02x:%02x:%02x:%02x req_mode=00 %s",
            g_wlan, dest_mac[0], dest_mac[1], dest_mac[2], dest_mac[3], dest_mac[4], dest_mac[5], buf);
    printf("cmd: %s\n", cmd_buf);
    system(cmd_buf);
    return 0;
}
					 ...
					 const char *ifname)
{
	...
	//snprintf(buf, sizeof(buf), "ubus call hostapd.%s rrm_beacon_req ", ifname);

	if (parse_send_frame_params_mac("Dest_MAC", cmd, dut, buf, sizeof(buf)))
		return -1;

    ubus_cmd_add_name("mode", buf);
	str_val = get_param(cmd, "MeaMode");
	if (!str_val) {
		sigma_dut_print(dut, DUT_MSG_ERROR,
				"MeaMode parameter not present in send bcn-rpt-req");
    ...
    ubus_cmd_add_name("op_class", buf);
	if (parse_send_frame_params_int("RegClass", cmd, dut, buf, sizeof(buf)))
		return -1;

    ubus_cmd_add_name("channel", buf);
	if (parse_send_frame_params_int("Channel", cmd, dut, buf, sizeof(buf)))
		return -1;

    ubus_cmd_add_name("duration", buf);
	if (parse_send_frame_params_int("MeaDur", cmd, dut, buf, sizeof(buf)))
		return -1;

    ubus_cmd_add_name("bssid", buf);
	if (parse_send_frame_params_mac("BSSID", cmd, dut, buf, sizeof(buf)))
		return -1;

    ubus_cmd_add_name("ssid", buf);
	if (parse_send_frame_params_str("SSID", cmd, dut, buf, sizeof(buf)))
		return -1;

    sprintf(ubus_cmd, "ubus call hostapd.%s rrm_beacon_req '{%s}'", ifname, buf);
    printf("%s\n", ubus_cmd);
    system(ubus_cmd);
	return 0;
}
...
static int siflower_ap_send_frame_btm_req(struct sigma_dut *dut,
				     struct sigma_cmd *cmd, const char *ifname)
{
	char buf[512] = "";
    unsigned char mac[6];
    ...
    //通过寄存器将需要关beacon的需求传给lmac，然后停发beacon。需要lmac配合修改
    if(BTM_bcn_down)
        //现已封装成接口，不再用寄存器裸写
        system("echo 1 1 > /sys/kernel/debug/ieee80211/phy0/siwifi/wfa_setting");
        //system("devmem 0xc1e0008c 32 0x1");

    val = get_param(cmd, "Dest_MAC");
    if(val) {
	    if (parse_mac_address(dut, val, mac) < 0) {
		    sigma_dut_print(dut, DUT_MSG_ERROR, "MAC Address not in proper format");
		    return -1;
        }
    }

    if(BTM_DisAssoc_Imnt) {
        btm_add_int("disassoc_imminent", 1, buf);
        //4.2.5.3有问题，对面sta回复disassoc非常快，测试要求我们发disassoc而sta不能发，因此我们需要抢在sta之前发送，100太慢了，因此填1过测
        if(BTM_case5) {
            btm_add_int("disassoc_timer", 1, buf);
        //4.2.5.3之外的case，disassoc_timer填的100
        } else {
            btm_add_int("disassoc_timer", 100, buf);
        }
    } else {
        btm_add_int("disassoc_imminent", 0, buf);
        //hostapd里面不管disassoc_imminent字段，只要disassoc_timer里有值就经过这个值x100ms断开，0就不断（不发disassoc）
        //别人家的这个值单位应该是1ms，很多配的1000
        btm_add_int("disassoc_timer", 0, buf);
    }

    //115,15都有用到,case没有检查，但最后过测的时候用的15过的
    btm_add_int("valid_int", 15, buf);
    //测试时别人的token都是会变的，但是测试要求里没有规定，所以暂时写1
    btm_add_int("dialog_token", 1, buf);

    if(Term_Bit)
         btm_add_str("bss_term", ",2", buf);

    btm_add_neighbor(dut, buf);

    btm_add_int("pref", 1, buf);

    if(BTM_DisAssoc_Imnt)
        btm_add_str("mbo", "0:1:0", buf);
    else
        btm_add_str("mbo", "0:0:0", buf);

    sprintf(ubus_cmd, "hostapd_cli -i wlan%d bss_tm_req %02x:%02x:%02x:%02x:%02x:%02x %s", g_wlan, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], buf);
    printf("%s\n", ubus_cmd);
    system(ubus_cmd);
	return 0;
}
...
static int siflower_cmd_dev_send_frame(struct sigma_dut *dut, struct sigma_conn *conn,
				 struct sigma_cmd *cmd)
{
	const char *val;
    char ifname_1[10];
    ...
    sprintf(ifname_1, "wlan%d", g_wlan);
    ifname = (const char *)ifname_1;
	...
	if (strcasecmp(val, "BTMReq") == 0)
		siflower_ap_send_frame_btm_req(dut, cmd, ifname);
		//ath_ap_send_frame_btm_req(dut, cmd, ifname);
	else if (strcasecmp(val, "BcnRptReq") == 0)
		siflower_ap_send_frame_bcn_rpt_req(dut, cmd, ifname);
	else if (strcasecmp(val, "disassoc") == 0)
		ath_ap_send_frame_disassoc(dut, cmd, ifname);
	else
		return -1;
...
//siflower api register
void ap_register_siflower_cmds(void)
{
    //8.2
	sigma_dut_reg_cmd("ap_ca_version", NULL, siflower_cmd_ap_ca_version);
    //8.3
	sigma_dut_reg_cmd("ap_config_commit", NULL, siflower_cmd_ap_config_commit);
    //8.4
	sigma_dut_reg_cmd("ap_deauth_sta", NULL, siflower_cmd_ap_deauth_sta);
    //8.5
	sigma_dut_reg_cmd("ap_get_mac_address", NULL, siflower_cmd_ap_get_mac_address);
    //8.6
	sigma_dut_reg_cmd("ap_get_parameter", NULL, siflower_cmd_ap_get_parameter);
    //8.7
	sigma_dut_reg_cmd("ap_preset_testparameters", NULL, siflower_cmd_ap_preset_testparameters);
    //8.8
	sigma_dut_reg_cmd("ap_reset_default", NULL, siflower_cmd_ap_reset_default);
    //8.9
	//sigma_dut_reg_cmd("ap_send_addba_req", NULL, siflower_cmd_ap_send_addba_req);
    //8.10
	//sigma_dut_reg_cmd("ap_send_bcnrpt_req", NULL, siflower_cmd_ap_send_bcnrpt_req);
    //8.11
	//sigma_dut_reg_cmd("ap_send_bsstrans_mgmt_req", NULL, siflower_cmd_ap_send_bsstrans_mgmt_req);
    //8.12
	//sigma_dut_reg_cmd("ap_send_link_mea_req", NULL, siflower_cmd_ap_send_link_mea_req);
    //8.13
	//sigma_dut_reg_cmd("ap_send_link_tsmrpt_req", NULL, siflower_cmd_ap_send_tsmrpt_req);
    //8.14
	sigma_dut_reg_cmd("ap_set_11d", NULL, siflower_cmd_ap_set_11d);
    //8.15
	sigma_dut_reg_cmd("ap_set_11h", NULL, siflower_cmd_ap_set_11h);
    //8.16
	sigma_dut_reg_cmd("ap_set_11n_wireless", NULL, siflower_cmd_ap_set_11n_wireless);
    //8.17
	sigma_dut_reg_cmd("ap_set_apqos", NULL, siflower_cmd_ap_set_apqos);
    //8.18
	//sigma_dut_reg_cmd("ap_set_hs2", NULL, siflower_cmd_ap_set_hs2);
    //8.19
	//sigma_dut_reg_cmd("ap_set_qos", NULL, siflower_cmd_ap_set_qos);
    //8.20
	sigma_dut_reg_cmd("ap_set_pmf", NULL, siflower_cmd_ap_set_pmf);
    //8.21
	//sigma_dut_reg_cmd("ap_set_radius", NULL, siflower_cmd_ap_set_radius);
    //8.22
	sigma_dut_reg_cmd("ap_set_rfeature", NULL, siflower_cmd_ap_set_rfeature);
    //8.23
	//sigma_dut_reg_cmd("ap_set_rrm", NULL, siflower_cmd_ap_set_rrm);
    //8.24
	sigma_dut_reg_cmd("ap_set_security", NULL, siflower_cmd_ap_set_security);
    //8.25
	//sigma_dut_reg_cmd("ap_set_staqos", NULL, siflower_cmd_ap_set_staqos);
    //8.26
	sigma_dut_reg_cmd("ap_set_wireless", NULL, siflower_cmd_ap_set_wireless);
    //8.27
	//sigma_dut_reg_cmd("ap_wps_enter_pin", NULL, siflower_cmd_ap_wps_enter_pin);
    //MBO
	sigma_dut_reg_cmd("dev_send_frame", NULL, siflower_cmd_dev_send_frame);

}

//TODO, not finished 没完成的接口假装也完成了，这样万一测试的时候用到了也不会立刻停下来，可以多打一些日志
#define NF(name) \
static enum sigma_cmd_result siflower_cmd_ap_##name(struct sigma_dut *dut, struct sigma_conn *conn, struct sigma_cmd *cmd) { \
	return SUCCESS_SEND_STATUS; \
}

NF(send_addba_req)
NF(send_bcnrpt_req)
NF(send_bsstrans_mgmt_req)
NF(send_link_mea_req)
NF(send_tsmrpt_req)
NF(set_hs2)
NF(set_qos)
NF(set_radius)
...
void ap_register_siflower_cmds_not_finish(void){
    //8.9
	sigma_dut_reg_cmd("ap_send_addba_req", NULL, siflower_cmd_ap_send_addba_req);
    //8.10
	sigma_dut_reg_cmd("ap_send_bcnrpt_req", NULL, siflower_cmd_ap_send_bcnrpt_req);
	...
	sigma_dut_reg_cmd("ap_send_link_mea_req", NULL, siflower_cmd_ap_send_link_mea_req);
    //8.13
	sigma_dut_reg_cmd("ap_send_tsmrpt_req", NULL, siflower_cmd_ap_send_tsmrpt_req);
    //8.18
	sigma_dut_reg_cmd("ap_set_hs2", NULL, siflower_cmd_ap_set_hs2);
    //8.19
	sigma_dut_reg_cmd("ap_set_qos", NULL, siflower_cmd_ap_set_qos);
