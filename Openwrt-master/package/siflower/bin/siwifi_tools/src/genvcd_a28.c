...
static int add_group(struct groups *groups, char *name, char *id_prefix, int lsb, int msb, uint32_t val, int start)
{
    struct group *g;
...
static FILE *plfdiag2_config_open(char *path)
{
    FILE *fp;
    char filename[256];

    snprintf(filename, 255, "%s/plfdiags2", path);
    fp = fopen(filename, "r");

    return fp;
}

static FILE *plfdiag3_config_open(char *path)
{
    FILE *fp;
    char filename[256];

    snprintf(filename, 255, "%s/plfdiags3", path);
    fp = fopen(filename, "r");
...
static int get_la_conf(uint32_t *sampling, uint32_t *version, char *path, char *name, uint32_t *mode)
{
    FILE *fp;
    char filename[256];
    int i;
...
static int create_swdiag_groups(struct groups *swdiags, char *path, int sw_idx, int idx)
{
    ...
    {
       sw_idx = sw_idx - 2;
       snprintf(filename, 255, "%s/swdiags1", path);
       fp = fopen(filename, "r");
    }
    if (fp == NULL)
        return -1;
    ...
    while (1)
    {
        ...
        if (strcmp(line_prev, line))
        {
            ...
            /* Create new group */
            add_group(swdiags, line, grouptag, i - 16 * sw_idx, i - 16 * sw_idx, 0, 0);
    ...
    for (;i < 16 * (sw_idx + 1); i++)
    {
        ...
        add_group(swdiags, name, grouptag, i, i, 0, 0);
    }
...
static int create_mpif_groups(struct groups *mpif)
{
    ...
    add_group(mpif, "mpif_rifsDetected",     "mp", 0,  0,  0, 0);
    add_group(mpif, "mpif_keepRfOn",         "mp", 1,  1,  0, 0);
    add_group(mpif, "mpif_phyErr",           "mp", 2,  2,  0, 0);
    add_group(mpif, "mpif_rxEnd_p",          "mp", 3,  3,  0, 0);
    add_group(mpif, "mpif_rxErr_p",          "mp", 4,  4,  0, 0);
    add_group(mpif, "mpif_rxEndForTiming_p", "mp", 5,  5,  0, 0);
    add_group(mpif, "mpif_ccaSecondary",     "mp", 6,  6,  0, 0);
    add_group(mpif, "mpif_ccaPrimary",       "mp", 7,  7,  0, 0);
    add_group(mpif, "mpif_rxReq",            "mp", 16, 16, 0, 0);
    add_group(mpif, "mpif_txEnd_p",          "mp", 17, 17, 0, 0);
    add_group(mpif, "mpif_txReq",            "mp", 28, 28, 0, 0);
    add_group(mpif, "mpif_rfshutdown",       "mp", 29, 29, 0, 0);
    add_group(mpif, "mpif_phyRdy_u1",        "mp", 30, 30, 0, 0); //will be always 0 for SF16A18 PLATFORM
    add_group(mpif, "mpif_macDataValid_u1",  "mp", 31, 31, 0, 0); //will be always 0 for SF16A18 PLATFORM
    add_group(mpif, "mpif_rxData",           "mp", 8,  15, 0, 0);
    add_group(mpif, "mpif_phyRdy",           "mp", 18, 18, 0, 0);
    add_group(mpif, "mpif_macDataValid",     "mp", 19, 19, 0, 0);
    add_group(mpif, "mpif_txData",           "mp", 20, 27, 0, 0);
...
static int create_hwdiag_fpgaa_groups(struct groups *hwdiags, char *path)
{
    ...
    for (i = 0; i < diag_cnt; i++)
    {
        ...
        for (j = 0; j < bank_len; j++)
        {
            ...
            {
                /* Create new group */
                add_group(hwdiags, line, "hwa", lsb, lsb, 0, 0);
                strcpy(line_prev, line);
            }
            else
            {
...
static int get_start_number(char **lineptr)
{
    char *line;
    line = *lineptr;
    int start;
    ...
    else if (size == 3)
    {
        if (flag == 1)
          ...
          return ((line[end - 2] - '0') * 10) + (line[end - 1] - '0');
    }
    else
    {
        printf("size is %d, end is %d start is %d\n", size, end , start);
    }
}
...
static int create_siwifidiag_groups(struct groups *hwdiags, char *path, int idx)
{
	...
	if(idx < 2)
		...
		config = plfdiag2_config_open(path);
    else if(idx < 7)
		config = plfdiag3_config_open(path);
	else
		return -1;
		...
		else
		{
			...
			for (j = 0; j < bank_len; j++)
			{
				...
				if (strcmp(line_prev, line))
				{
                    ...
                    tmp = get_start_number(&line);
					add_group(hwdiags, line, grouptag, lsb, lsb, 0, tmp);
					strcpy(line_prev, line);
				}
				else
				{
...
static int create_hwdiag_fpgab_groups(struct groups *hwdiags, char *path)
{
    ...
    for (i = 0; i < diag_cnt; i++)
    {
        ...
        for (j = 0; j < bank_len; j++)
        {
            ...
            {
                /* Create new group */
                add_group(hwdiags, line, "hwb", lsb, lsb, 0, 0);
                strcpy(line_prev, line);
            }
            else
            {
...
static int create_control_groups(struct groups *control,uint32_t version)
{
    ...
    if (version < 0x40000)
    {
        ...
        add_group(control, "trigger", "ct", 31,  31,  0 , 0);
    } else {
        printf(" with 6 triggers\n");
        add_group(control, "ext_trigger", "ct", 11,  14,  0, 0);
        add_group(control, "sw_trigger", "ct", 10,  10,  0, 0);
        add_group(control, "int_trigger", "ct", 9,  9,  0, 0);
    }
    return 0;
}
...
static void declare_groups(FILE *vcd, struct groups *groups)
{
    ...
    for (i = 0; i < groups->cnt; i++)
    {
        ...
        else
        {
            ...
            fprintf(vcd, "$var reg %d %s %s [%d:%d] $end\n", width, g->id, g->name, g->start + width - 1, g->start);
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    // for seven mode
	struct groups siwifidiag3;
	struct groups siwifidiag4;
	struct groups siwifidiag5;
	struct groups siwifidiag6;
	...
	get_la_conf(sampling, &la_version_mac, argv[0], "lamacconf", &la_splice_mode);
	...
	printf("LA Info.\n Sampling Freq : %i MHz\n Version : %i.%i\n LA_splice_mode: %d\n",
            1000000/delay, (la_version_mac&0xFF0000) >> 16, (la_version_mac&0xFFFF), la_splice_mode);
    if (la_splice_mode == 2)
    {
        sampling[1] |= 0xFFFF0000;
    ...
    else if ((la_splice_mode == 0) || (la_splice_mode == 3))
    {
        sampling[3] |= 0xFFFF0000;
		create_siwifidiag_groups(&siwifidiag0, argv[0], 0);
		create_siwifidiag_groups(&siwifidiag1, argv[0], 1);
		create_siwifidiag_groups(&siwifidiag2, argv[0], 2);
		create_siwifidiag_groups(&siwifidiag3, argv[0], 3);
		create_siwifidiag_groups(&siwifidiag4, argv[0], 4);
		create_siwifidiag_groups(&siwifidiag5, argv[0], 5);
		create_siwifidiag_groups(&siwifidiag6, argv[0], 6);
    }
    else if (la_splice_mode == 1)
    {
        sampling[0] |= 0xFFFF0000;
		create_siwifidiag_groups(&siwifidiag0, argv[0], 0);
    }
    else
    {
        printf("la_splice_mode is wrong,it cant be %d\n", la_splice_mode);
        return 0;
    }
    ...
    {
	   declare_groups(vcd, &controls);
	   declare_groups(vcd, &siwifidiag6);
       declare_groups(vcd, &siwifidiag4);
	   declare_groups(vcd, &siwifidiag5);
	   declare_groups(vcd, &siwifidiag2);
	   declare_groups(vcd, &siwifidiag3);
	   declare_groups(vcd, &siwifidiag0);
	   declare_groups(vcd, &siwifidiag1);
    }
    else
    ...
    {
	   init_groups(vcd, &controls, (sampling[3] & 0xFFFF0000) >> 16);
	   init_groups(vcd, &siwifidiag6, sampling[3] & 0x0000FFFF);
       init_groups(vcd, &siwifidiag4, sampling[2] & 0x0000FFFF);
	   init_groups(vcd, &siwifidiag5, (sampling[2] & 0xFFFF0000) >> 16);
       init_groups(vcd, &siwifidiag2, sampling[1] & 0x0000FFFF);
	   init_groups(vcd, &siwifidiag3, (sampling[1] & 0xFFFF0000) >> 16);
	   init_groups(vcd, &siwifidiag0, sampling[0] & 0x0000FFFF);
	   init_groups(vcd, &siwifidiag1, (sampling[0] & 0xFFFF0000) >> 16);
    }
    else
    {
	   init_groups(vcd, &controls, (sampling[0] & 0xFFFF0000) >> 16);
	   init_groups(vcd, &siwifidiag0, sampling[0] & 0x0000FFFF);
    }
	...
	while(1)
	{
      ...
      else if ((la_splice_mode == 0) || (la_splice_mode == 3))
      {
		...
		buf4[8] = 0;
		pos = NULL;
		value[3] = strtoul(buf3, &pos,16);
		value[2] = strtoul(buf4, &pos,16);
		...
		{
			//128bits: sample_type(1) + reserved(103) + time_cnt(24)
			time += (unsigned long long)(value[0] & 0x00FFFFFF) * delay;
		}

		if(pre_value[0] != 0 || pre_value[1] != 0 || pre_value[2] != 0 || pre_value[3] != 0)
		{
			//128bits: sample_type(1) + trigger_status(6) + time_cnt(9) + data(112)
			time += (unsigned long long)((pre_value[3] & 0x01FF0000) >> 16) * delay;
			put_groups(vcd, &controls, (pre_value[3] & 0x7E000000) >> 16, (sampling[3] & 0xFFFF0000) >> 16);
			put_groups(vcd, &siwifidiag6, pre_value[3] & 0xFFFF, sampling[3] & 0x0000FFFF);
            put_groups(vcd, &siwifidiag5, pre_value[2] >> 16, (sampling[2] & 0xFFFF0000) >> 16);
			put_groups(vcd, &siwifidiag4, pre_value[2] & 0xFFFF, sampling[2] & 0x0000FFFF);
			put_groups(vcd, &siwifidiag3, pre_value[1] >> 16, (sampling[1] & 0xFFFF0000) >> 16);
            put_groups(vcd, &siwifidiag2, pre_value[1] & 0xFFFF, sampling[1] & 0x0000FFFF);
			put_groups(vcd, &siwifidiag1, pre_value[0] >> 16, (sampling[0] & 0xFFFF0000) >> 16);
			put_groups(vcd, &siwifidiag0, pre_value[0] & 0xFFFF, sampling[0] & 0x0000FFFF);
      ...
      else
      {
        ...
        for (int j=0; j<2; j++)
        {
		 if((value[j] & 0x80000000))
		 {
			//32bits: sample_type(1) + reserved(15) + time_cnt(16)
			time += (unsigned long long)(value[j] & 0x0000FFFF) * delay;
		 }

		 if(pre_value[0] != 0)
		 {
			//32bits: sample_type(1) + trigger_status(6) + time_cnt(9) + data(16)
			time += (unsigned long long)((pre_value[0] & 0x01FF0000) >> 16) * delay;
			put_groups(vcd, &controls, (pre_value[0] & 0x7E000000) >> 16, (sampling[0] & 0xFFFF0000) >> 16);
			put_groups(vcd, &siwifidiag0, pre_value[0] & 0xFFFF, sampling[0] & 0x0000FFFF);
