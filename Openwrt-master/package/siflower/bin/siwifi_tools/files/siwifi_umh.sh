...
dmesg > $DIRPATH/dmesg
# set platform, for now just consider HE_AP
echo HE_AP > $DIRPATH/siwifiplat
...
if [[ $2 =~ trace_only$ ]]
then
echo "$(date +"%F-%Hh%Mm%S")--$1--` cat $siwifi_diag/trace_error`" >> $PUBDIRPATH/error_list.txt
cat $siwifi_diag/dump_fw_trace > $DIRPATH/$(basename dump_fw_trace)
else
for i in $siwifi_diag/*
do
if [[ $i =~ /fw_trace$ ]]
then
echo "$i"
else
    cat $i > $DIRPATH/$(basename $i)
fi
done
...
hexdump -v -e '"%08x""\n"' $DIRPATH/dump_fw_trace > $DIRPATH/fw_trace.txt
